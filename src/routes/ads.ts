...
router.post("/api/ads", async (ctx) => {
  ...
  if (title.length > MAX_TITLE_LENGTH) {
    ctx.response.status = 400
    ctx.response.body = { error: `Titeln får inte vara längre än ${MAX_TITLE_LENGTH} tecken` }
    return
  }
  ...
  if (description.length > MAX_DESCRIPTION_LENGTH) {
    ctx.response.status = 400
    ctx.response.body = {
      error: `Beskrivningen får inte vara längre än ${MAX_DESCRIPTION_LENGTH} tecken`,
    }
    return
  }
